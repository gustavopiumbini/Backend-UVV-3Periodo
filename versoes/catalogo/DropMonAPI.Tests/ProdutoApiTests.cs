...
public sealed class ProdutoApiTests : IDisposable
{
    ...
    [Fact]
    public async Task CatalogoEEstoque_ExigemLogin_QuandoProtecaoEstaAtiva()
    {
        using var protegida = new ApiFactory(requireAuthentication: true);
        using var client = protegida.CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
        Assert.Equal(HttpStatusCode.Unauthorized, (await client.GetAsync("/api/produtos")).StatusCode);
        Assert.Equal(HttpStatusCode.Unauthorized, (await client.GetAsync("/media/00000000000000000000000000000000.webp")).StatusCode);

        using var request = new HttpRequestMessage(HttpMethod.Post, "/api/produtos") {
            Content = JsonContent.Create(ProdutoValido)
        };
        ...
        var status = await client.GetFromJsonAsync<JsonElement>("/api/sessao/status");
        Assert.True(status.GetProperty("authenticationRequired").GetBoolean());
    ...
    public void Migrations_GeramPostgresComIdentidadePrecisaoERls()
    {
        var options = new DbContextOptionsBuilder<AppDbContext>()
            .UseNpgsql("Host=localhost;Database=postgres;Username=postgres;Password=test")
            .Options;
        ...
        Assert.Contains("numeric(12,2)", sql, StringComparison.OrdinalIgnoreCase);
    ...
    private sealed class ApiFactory(bool requireAuthentication = false) : WebApplicationFactory<Program>
    {
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.UseSetting("Admin:RequireAuthentication", requireAuthentication.ToString());
            builder.UseSetting("DataProtection:KeysPath", Path.Combine(uploadDirectory, "keys"));
            ...
            builder.ConfigureServices(services =>
            {
                ...
                services.AddSingleton(sp => new DropMonAPI.Services.FotoStorage(
                    ...
                    new Microsoft.Extensions.Configuration.ConfigurationBuilder().AddInMemoryCollection(
                        new Dictionary<string, string?> { ["Storage:RootPath"] = uploadDirectory }).Build(),
                    sp.GetRequiredService<IHttpClientFactory>(),
                    sp.GetRequiredService<Microsoft.Extensions.Logging.ILogger<DropMonAPI.Services.FotoStorage>>()));
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing && Directory.Exists(uploadDirectory)) Directory.Delete(uploadDirectory, recursive: true);
            if (disposing)
